...
async function closeChannel(channelToBeClosed, reason, discordClient){
	...
	const modArray = await prisma.mods.findMany({
		where: {
			channelID: channelToBeClosed.id
		}
	})
	
	const guild = await prisma.settings.findFirst({
		where:{
			guildID: channelToBeClosed.guild.id
		}
	})
	
	const dbChannel = await prisma.channels.findFirst({
		where: {channelID: channelToBeClosed.id}
	})
	...
	discordClient.channels.fetch(guild.privateBotChannel).then((channel)=>{
		const embed = new Discord.MessageEmbed()
			.setColor('#0099ff')
			.setTitle('Closed Channel!')
			.setDescription(channelToBeClosed.name+" Moved to Closed Channels!")
